// See problem statement at https://techdevguide.withgoogle.com/paths/foundational/find-longest-word-in-dictionary-that-subsequence-of-given-string/#!
...
module.exports.findLongestSubstring = function (s, d){

  // sort dictionary by word length, longest first.
  d.sort((a,b)=>b.length-a.length);
  for(let w of d){
    if(isWordSubsequence(w,s))return w; // return first match
  }
  return "";
}


module.exports.findLongestSubstringV1 = function (s, d) {
  let longestW = "";

  for (let w of d) {
    if (isWordSubsequence(w, s)) {
...
function isWordSubsequence(w,s){
  let sIndex = 0;
  let wIndex = 0;
  while (sIndex < s.length && wIndex<w.length){
    // if characters are the same, increase w index
    if (w.charAt(wIndex)==s.charAt(sIndex)) wIndex++
    sIndex++; // always increase index of sequence
  }
